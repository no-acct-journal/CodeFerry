...
from codeferry.agents.parser import AgentDef, AgentParseError, parse_agent_file, parse_frontmatter
...
from codeferry.agents.fork import (
    FORK_BOILERPLATE_TAG,
    ForkError,
    build_forked_messages,
)
from codeferry.agents.trace import TraceManager, TraceNode
from codeferry.agents.task_manager import BackgroundTask, TaskManager
...
class TestAgentLoader:
    ...
    def test_project_overrides_builtin(self, tmp_path: Path):
        agents_dir = tmp_path / ".codeferry" / "agents"
...
class TestToolFilter:
    ...
    def test_builtin_no_custom_restrictions(self):
        # EnterPlanMode now belongs to ALL_AGENT_DISALLOWED to stay consistent with
        # the Go version, so use a tool that exists only in CUSTOM and not in ALL to
        # verify that built-in agents skip the custom layer. Because the Go version
        # clones ALL into CUSTOM, this only verifies that built-in agents still get
        # normal tools.
...
class TestForkMode:
    def test_basic_fork(self):
        ...
        assert len(messages) == 4  # 3 original messages + 1 fork task.
    ...
    def test_fork_wraps_pending_tool_use(self):
        ...
        forked = build_forked_messages(conv, "task")
        # Should contain: user, assistant+tool_use, placeholder tool_result, fork task.
    ...
    def test_fork_is_deep_copy(self):
        ...
        assert len(forked.history) == 3  # Original message + fork task + extra message.
...
class TestTaskManager:
    ...
    def mock_agent(self):
        ...
        # Regular non-team subagent: team_name is empty. Otherwise _run_background
        # enters the team idle loop (once per second, up to 60 times), the background
        # task never reaches notify_queue.put in finally, and poll_completed cannot
    ...
    async def test_list_tasks(self, mock_agent):
        ...
        assert names == {"t1", "t2"}
        await asyncio.sleep(0.1)  # Let the background tasks finish.
...
class TestAgentToolParams:
    def test_required_fields(self):
        from codeferry.tools.agent_tool import AgentToolParams
...
# =====================================================================
# 11. Agent: run_to_completion basics, agent_id, and trace_id
# =====================================================================

"""Unit tests for provider serialization builders.

The session layer is provider-agnostic; serialization lives in codeferry.serialization.
These tests lock down the various wire formats and, more importantly, the
Extended Thinking round-trip contract: a turn with tool-use must send its signed
thinking block back to the API, otherwise Anthropic returns 400.
"""
...
def test_openai_input_tool_use_as_function_call():
    ...
    assert len(msgs) == 2  # Text message + function_call.
...
def test_chat_completion_uses_tool_calls_and_skips_thinking():
    ...
    # The Chat Completions format has no place to carry a thinking block.

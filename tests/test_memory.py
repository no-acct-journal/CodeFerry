...
from codeferry.memory.instructions import (
    MAX_INCLUDE_DEPTH,
    load_instructions,
    process_includes,
)
...
# =========================================================================
# A. Instruction files (codeferry.md)
# =========================================================================
...
class TestLoadInstructions:
    def test_single_layer(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        codeferry_md = tmp_path / "codeferry.md"
        codeferry_md.write_text("project instructions", encoding="utf-8")
    ...
    def test_multi_layer_priority(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        root_md = tmp_path / "codeferry.md"
        root_md.write_text("root level", encoding="utf-8")
        dotdir = tmp_path / ".codeferry"
        dotdir.mkdir()
        dot_md = dotdir / "codeferry.md"
...
# =========================================================================
# D. Message-chain validation and session resume
# =========================================================================
...
# =========================================================================
# D2. Compact-boundary persistence and reloading compacted state on resume
# =========================================================================
...
class TestCompactBoundaryRoundTrip:
    def test_make_and_parse_boundary_text_only(self) -> None:
        ...
        # JSONL round-trip serialization; content is a dict and must serialize/deserialize fully.
        line = rec.to_jsonl()
        restored = SessionRecord.from_jsonl(line)
        assert restored is not None
    ...
    def test_boundary_preserves_tool_pairs_in_keep(self) -> None:
        # Kept tail messages include a tool_use <-> tool_result pair and must be preserved fully.
        keep = [
            Message(
                role="assistant",
                content="running",
    ...
    def test_resume_rebuilds_compacted_state(self, tmp_path: Path) -> None:
        """Core round-trip flow: raw prefix + boundary (summary + kept messages) + post-boundary messages.

        Resume must rebuild the compacted state: the summary exists, kept messages are
        preserved verbatim, the raw prefix before the boundary is not replayed, and
        messages appended after the boundary are present normally.
        """
        mgr = SessionManager(str(tmp_path))
        s = mgr.create()
        sid = s.session_id

        # Raw prefix that was summarized and should not be replayed.
        ...
        # The boundary inlines the summary and the verbatim kept tail messages.
        keep = [
        ...
        # Continuation after the boundary.
        ...
        # Summary exists, represented as a user message.
        assert any("SUMMARY OF OLD STUFF" in c for c in contents)
        # Kept tail messages exist verbatim.
        assert "KEPT recent question" in contents
        assert "KEPT recent answer" in contents
        # Continuation after the boundary exists.
        assert "NEW followup" in contents
        assert "NEW reply" in contents
        # Raw prefix before the boundary was not replayed.
        assert all("OLD raw" not in c for c in contents)

        # Structural order: summary first, then kept messages, then post-boundary messages.
    ...
    def test_resume_uses_last_boundary_when_multiple(self, tmp_path: Path) -> None:
        """Chained compaction: only the last boundary determines the resumed state."""
        mgr = SessionManager(str(tmp_path))
        s = mgr.create()
        sid = s.session_id
        ...
        # All content from the first compaction generation has disappeared.
    ...
    def test_resume_no_boundary_full_replay(self, tmp_path: Path) -> None:
        """Backward compatibility: sessions without boundaries still replay fully."""
        mgr = SessionManager(str(tmp_path))
        s = mgr.create()
        sid = s.session_id
    ...
    def test_append_record_does_not_bump_message_count(self, tmp_path: Path) -> None:
        ...
        assert s.meta.message_count == before  # The boundary is only a marker, not a conversation turn.
        s.close()

# =========================================================================
# E. Time-gap prompt message
# =========================================================================
...
class TestTimeGapMessage:
    ...
    def test_gap_returns_message(self) -> None:
        ...
        assert "code may have changed" in msg.content

# =========================================================================
# F. Session metadata SessionMeta
# =========================================================================
...
class TestMemoryManager:
    ...
    def test_load_merges_user_and_project(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        user_mem = fake_home / ".codeferry" / "memories.md"
        user_mem.parent.mkdir(parents=True)
        user_mem.write_text("### User Preferences\n- prefer spaces", encoding="utf-8")

        project_mem = tmp_path / "project" / ".codeferry" / "memories.md"
        project_mem.parent.mkdir(parents=True)
        project_mem.write_text("### Project Knowledge\n- uses PostgreSQL", encoding="utf-8")
    ...
    def test_get_display_text_empty(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(Path, "home", classmethod(lambda cls: fake_home))
        mgr = MemoryManager(str(tmp_path / "project"))
        assert "No automatic memories" in mgr.get_display_text()
...
# =========================================================================
# H. Injecting long-term memory into a conversation
# =========================================================================
...
class TestConversationInjection:
    def test_inject_long_term_memory(self) -> None:
        ...
        assert "codeferryMd" in conv.history[1].content
    ...
    def test_inject_instructions_only(self) -> None:
        ...
        assert "codeferryMd" in conv.history[0].content
...
class TestMemoryExtraction:
    def test_extraction_prompt_contains_categories(self, tmp_path: Path) -> None:
        from codeferry.memory.auto_memory import MEMORY_EXTRACTION_PROMPT

        assert "User Preferences" in MEMORY_EXTRACTION_PROMPT
        assert "Correction Feedback" in MEMORY_EXTRACTION_PROMPT
        assert "Project Knowledge" in MEMORY_EXTRACTION_PROMPT
        assert "References" in MEMORY_EXTRACTION_PROMPT
        assert "Do not add duplicates" in MEMORY_EXTRACTION_PROMPT

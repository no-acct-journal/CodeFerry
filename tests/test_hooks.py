...
class TestPromptExecutor:
    ...
    async def test_returns_message(self):
        from codeferry.hooks.executors import execute_prompt
...
class TestHttpExecutor:
    ...
    async def test_mock_request(self):
        from codeferry.hooks.executors import execute_http

        action = Action(type="http", url="https://httpbin.org/post", body='{"test": true}')
        ctx = HookContext()
        # Use a mock to avoid making a real network request.
        with patch("codeferry.hooks.executors.urlopen") as mock_urlopen:
...
class TestAgentExecutor:
    ...
    async def test_stub(self):
        from codeferry.hooks.executors import execute_agent
...
class TestExecuteAction:
    ...
    async def test_dispatch(self):
        from codeferry.hooks.executors import execute_action
...
class TestAgentHookIntegration:
    """Verify that a pre_tool_use rejection causes the tool call to be skipped."""

    @pytest.mark.asyncio
    async def test_pre_tool_use_reject_skips_tool(self):
        from codeferry.agent import Agent, ToolResultEvent

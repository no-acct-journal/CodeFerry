...
from codeferry.tools.base import StreamEnd, StreamEvent, TextDelta, ToolCallComplete
...
class TestPathSandbox:
    ...
    def test_temp_dir_allowed(self) -> None:
        tmp = Path(tempfile.gettempdir()) / "codeferry_test.txt"
...
class TestRuleEngine:
    ...
    def test_append_local_rule(self) -> None:
        tmpdir = Path(tempfile.mkdtemp())
        local_path = tmpdir / ".codeferry" / "permissions.local.yaml"
...
# ===========================================================================
# Layer 5 (integrated): PermissionChecker working across all five layers
# ===========================================================================
...
# ===========================================================================
# Integration tests: Agent + permission system (end-to-end)
# ===========================================================================
...
async def test_e2e_dangerous_command_blocked_loop_continues():
    """A dangerous command is blocked, the error is returned to the model, and the loop continues."""
    tmpdir = Path(tempfile.mkdtemp())
    client = MockLLMClient([
        # Turn 1: the model tries to run rm -rf /.
        [
            TextDelta("Let me clean up."),
            ToolCallComplete("t1", "Bash", {"command": "rm -rf /"}),
            StreamEnd("end_turn", input_tokens=10, output_tokens=20),
        ],
        # Turn 2: the model adjusts its strategy.
        [
    ...
    assert "denied" in c["tool_result"][0].output.lower() or "dangerous" in c["tool_result"][0].output.lower()
    assert len(c["loop"]) == 1
...
async def test_e2e_sandbox_blocks_outside_path():
    ...
    assert len(c["tool_result"]) == 1
    assert c["tool_result"][0].is_error
    assert "sandbox" in c["tool_result"][0].output

@pytest.mark.asyncio
async def test_e2e_rule_allows_git():
    """A rule that allows git commands lets them pass without HITL."""
    tmpdir = Path(tempfile.mkdtemp())
...
async def test_e2e_default_mode_write_triggers_ask():
    """In default mode, write tools produce an ASK decision and trigger a PermissionRequest event."""
    tmpdir = Path(tempfile.mkdtemp())
    client = MockLLMClient([
        [
...
async def test_e2e_user_denies_operation():
    """The user denies an operation through HITL, and the model receives an error and adjusts."""
    tmpdir = Path(tempfile.mkdtemp())
    client = MockLLMClient([
        [

...
"""Tests for ContentReplacementState: plan B with frozen decisions and no in-place mutation."""
...
# ---------------------------------------------------------------------------
# Plan B: apply does not mutate the input conversation.
# ---------------------------------------------------------------------------
...
def test_apply_does_not_mutate_conv(tmp_path: Path) -> None:
    ...
    # The original conversation must remain unchanged (the plan B invariant).
    assert conv.history[0].tool_results[0].content == orig_content
    # api_conv is another ConversationManager backed by a different list.
    assert api_conv is not conv
    assert api_conv.history is not conv.history
    # It carries the replaced content.
...
def test_first_call_freezes_unreplaced(tmp_path: Path) -> None:
    """Results within budget must be marked as seen but not added to replacements."""
    small = "x" * 100
...
def test_replacement_byte_identical(tmp_path: Path) -> None:
    """Calling apply twice on the same conversation should produce byte-identical api_conv content."""
    big = "x" * (SINGLE_RESULT_CHAR_LIMIT + 100)
    ...
    # The second pass is a pure re-apply: no new records and no new files.
    assert recs2 == []

# ---------------------------------------------------------------------------
# Decision freezing: once marked as seen but unreplaced, an id is never replaced later.
# ---------------------------------------------------------------------------

def test_frozen_never_replaced(tmp_path: Path) -> None:
    """An id marked as unreplaced in turn 1 must never be selected for replacement later,
    even if a later aggregate size would otherwise pick it."""
    # Turn 1: a single roughly 4K result, far below the aggregate limit.
    ...
    # Turn 2: simulate the same message growing by appending parallel tool results,
    # causing the aggregate size to exceed the budget. This should never happen in
    # reality because messages are immutable once added; this forced construction
    # verifies the invariant.
    fresh_large = "b" * (quarter * 3 + 100)  # A very large new candidate.
    ...
    # The first pass spills t2 individually because it exceeds SINGLE_RESULT_CHAR_LIMIT,
    # so t1 keeps its original content regardless of aggregate size. The key point is
    # that t1 is never reconsidered.
...
def test_aggregate_only_picks_fresh(tmp_path: Path) -> None:
    """When aggregate size exceeds the budget and only fresh candidates qualify, frozen ids are untouched even if largest."""
    # All results are below SINGLE_RESULT_CHAR_LIMIT, but the aggregate exceeds AGGREGATE.
    ...
    # Some results are replaced; total size is now within the limit.
    ...
    # All ids should now be in seen_ids because each has a decision.
...
def test_reconstruct_from_records() -> None:
    ...
    records = [
        ...
        # t2 has no record, so after reconstruction it is frozen and unreplaced.
    ]
...
def test_reconstruct_with_inherited_parent() -> None:
    """Fork continuation: fill missing record ids from the parent's current replacements."""
    msgs = [
        Message(
            role="user", content="",
            tool_results=[
...
def test_append_and_load_records_roundtrip(tmp_path: Path) -> None:
    ...
    # The file is JSONL, one object per line.

...
def test_recovery_attachment_emits_all_sections():
    ...
    assert "- Bash" in out
    assert "Note" in out  # Title of the final note section.

def test_recovery_file_limit_and_order():
    state = RecoveryState()
    # Record 7 files with staggered timestamps; only the newest 5 should appear.
    for i in range(7):
        state.record_file_read(f"/f{i}", "x")
        # Force timestamps so the order is deterministic.
        rec = state._files[f"/f{i}"]
        rec.timestamp = 1000.0 + i

    files = state.snapshot_files(RECOVERY_FILE_LIMIT)
    assert len(files) == 5
    assert files[0].path == "/f6"  # The newest file appears first.
...
def test_recovery_skills_budget():
    ...
    # 25K / 5K per skill means at most 5 skills.
    assert 1 <= emitted <= 5

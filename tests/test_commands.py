"""Slash Command framework tests: registry, parser, completion, and handlers."""
...
class TestParseCommand:
    ...
    def test_command_with_args(self) -> None:
        name, args, is_cmd = parse_command("/compact keep database-related content")
        assert is_cmd is True
        assert name == "compact"
        assert args == "keep database-related content"
...
class TestHelpHandler:
    ...
    async def test_list_all(self) -> None:
        ...
        from codeferry.commands.handlers.help import handle_help
        ...
        assert "Available commands" in ui.messages[0]
    ...
    async def test_help_unknown_command(self) -> None:
        ...
        ctx.config = {"registry": registry}
        await handle_help(ctx)
        assert "Unknown command" in ui.messages[0]

class TestPlanDoHandlers:

    @pytest.mark.asyncio
    async def test_plan_switches_mode(self) -> None:
        from codeferry.commands.handlers.plan import handle_plan

        ui = MockUI()
        ctx = _make_context(args="", ui=ui)
        await handle_plan(ctx)
        assert ui._plan_mode is True
        assert "Plan mode" in ui.messages[0]
    ...
    async def test_plan_with_args_sends_message(self) -> None:
        ...
        ui = MockUI()
        ctx = _make_context(args="Design the login module", ui=ui)
        await handle_plan(ctx)
        assert ui._plan_mode is True
        assert "Design the login module" in ui.sent_messages

    @pytest.mark.asyncio
    async def test_do_switches_back(self) -> None:
        from codeferry.commands.handlers.do import handle_do

        ui = MockUI()
        ctx = _make_context(args="", ui=ui)
        await handle_do(ctx)
        assert ui._plan_mode is False
        assert "Execution mode" in ui.messages[0]

class TestSkillHandler:
    @pytest.mark.asyncio
    async def test_skill_list_no_loader(self) -> None:
        from codeferry.commands.handlers.skill import handle_skill

        ui = MockUI()
        ctx = _make_context(args="list", ui=ui)
        await handle_skill(ctx)
        assert "not initialized" in ui.messages[0]
    ...
    async def test_skill_list_with_loader(self) -> None:
        ...
        ctx = _make_context(args="list", ui=ui)
        loader = MagicMock()
        loader.get_catalog.return_value = [("commit", "Analyze git diff")]
    ...
    async def test_skill_unknown_subcmd(self) -> None:
        ...
        ctx.config = {"skill_loader": loader}
        await handle_skill(ctx)
        assert "Unknown subcommand" in ui.messages[0]

class TestStatusHandler:

    @pytest.mark.asyncio
    async def test_status_output(self) -> None:
        from codeferry.commands.handlers.status import handle_status
        ...
        assert "codeferry status" in ui.messages[0]
        assert "default" in ui.messages[0]

class TestSessionHandler:
    @pytest.mark.asyncio
    async def test_session_no_manager(self) -> None:
        from codeferry.commands.handlers.session import handle_session
    ...
    async def test_session_list_empty(self) -> None:
        ...
        await handle_session(ctx)
        assert "No saved sessions" in ui.messages[0]
    ...
    async def test_session_unknown_sub(self) -> None:
        ...
        await handle_session(ctx)
        assert "Usage" in ui.messages[0]

class TestMemoryHandler:
    @pytest.mark.asyncio
    async def test_memory_display(self) -> None:
        from codeferry.commands.handlers.memory import handle_memory

        ui = MockUI()
        mm = MagicMock()
        mm.get_display_text.return_value = "Memory content"
        ctx = _make_context(args="", ui=ui)
        ctx.memory_manager = mm
        await handle_memory(ctx)
        assert "Memory content" in ui.messages[0]
    ...
    async def test_memory_clear(self) -> None:
        ...
        assert "cleared" in ui.messages[0]
...
# ---------------------------------------------------------------------------
# Integration test: register_all_commands
# ---------------------------------------------------------------------------

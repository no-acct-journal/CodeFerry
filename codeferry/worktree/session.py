...
def _session_path(codeferry_dir: Path) -> Path:
    return codeferry_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(codeferry_dir)
...
def load_worktree_session(codeferry_dir: Path) -> WorktreeSession | None:
    path = _session_path(codeferry_dir)
    if not path.exists():
        return None
    try:

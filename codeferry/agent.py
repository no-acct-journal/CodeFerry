...
from codeferry.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from codeferry.conversation import ThinkingBlock as ConvThinkingBlock
...
from codeferry.hooks import HookContext, HookEngine, ToolRejectedError
from codeferry.hooks.engine import HookNotification
...
class CompactNotification:
    before_tokens: int
    message: str
    # Structured boundary (summary + preserved tail of original content).
    # Used by the UI/session layer to persist compact_boundary records.
...
# ---------------------------------------------------------------------------
# Streaming executor — starts tool execution during LLM streaming
# ---------------------------------------------------------------------------
...
class Agent:
    def __init__(
        ...
        # Store snapshots needed to rebuild working context after Layer 2
        # conversation compaction: recent file reads and skill invocations.
        # Recorded on each ReadFile / skill call and consumed when
        # auto_compact crosses its trigger threshold.
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".codeferry" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".codeferry" / "plans"
    ...
    async def run(self, conversation: ConversationManager) -> AsyncIterator[AgentEvent]:
        ...
        while True:
            ...
            # Layer 2: automatically compact when approaching the context
            # window limit (operates on the raw conversation).
            ...
            # Layer 1: apply the tool-result budget before the LLM call so
            # api_conv reflects all writes already made in this iteration
            # (system reminders, hook notifications, etc.).
            # The original conversation is not modified; replacement
            # decisions are stored in self.replacement_state.
            ...
            # After the assistant reply is added to history, anchor the
            # actual usage: the baseline (input + cache + output) now covers
            # the current position, so the next auto-compact check only
            # needs character estimates for newly appended tool results.
    ...
    async def _execute_tool(
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                # Yield a permission request event for the caller to handle.
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from codeferry.permissions.rules import Rule, extract_content
    ...
    def _snapshot_for_recovery(
        ...
        """Capture the content just handed to the model by ReadFile so
        auto_compact can reattach it after Layer 2 conversation compaction.
        The extra disk read per ReadFile is cheaper than reverse-parsing
        line numbers from tool output.
        """
    ...
    async def manual_compact(
        ...
        # auto_compact replaces conversation.history with a summary, so all
        # tool-result content (raw or already replaced) will be discarded.
        # Skip apply_tool_result_budget here: in the main loop its only job
        # is to generate api_conv for the LLM call, but this path does not
        # need an LLM call that sees replaced results (the summarization
        # call inside auto_compact operates on the raw conversation).
    ...
    async def run_to_completion(
        ...
        for iteration in range(1, self.max_iterations + 1):
            ...
            # The assistant reply is already in history, so anchor actual
            # usage here; the next iteration only needs character estimates
            # for the tool results appended below.
            conversation.record_usage_anchor(
                response.input_tokens,
                response.output_tokens,
                response.cache_read,
